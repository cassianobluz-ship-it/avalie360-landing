// /api/stripe-webhook.js — CommonJS puro, sem export/import ES modules
...
// NOTA: module.exports.config NÃO funciona junto com module.exports = function
// A Vercel lê o config apenas quando exportado como propriedade do handler
const handler = async (req, res) => {
  ...
  // ── PLANO PERSONALIZADO ──
  if (type === 'plan_custom') {
    console.log(`Ativando plan_custom para org: ${orgSlug} (id: ${orgId})`);
    ...
    try {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          html: `<p>Olá! O <strong>Plano Personalizado</strong> foi ativado para <strong>${orgName}</strong>.<br><br>
          Acesse o painel em <a href="https://avalie360.vercel.app">avalie360.vercel.app</a>, entre como administrador e clique em <strong>Formulários</strong> para começar a personalizar.</p>
          <p style="font-size:12px;color:#94a3b8">Avalie360 · Conectando Gente</p>`,
        }),
      });
    } catch(e) { console.error('Email plan_custom error:', e); }

    return res.status(200).json({ received: true });
  }

  // ── NOVA CONTRATAÇÃO ──
  console.log(`Nova contratação: ${orgSlug}`);
...
<div style="max-width:600px;margin:32px auto;background:white;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(37,99,235,.1)">
  ...
  <div style="padding:32px 36px">
    ...
    <div style="background:#eff6ff;border-radius:12px;padding:20px 24px;margin-bottom:24px;border-left:4px solid #2563eb">
      ...
      <div style="margin-bottom:12px"><span style="font-size:13px;color:#64748b">Identificador da organização:</span><br><strong style="font-size:15px;color:#0f172a">${orgSlug}</strong></div>
    ...
    <p style="font-size:13px;color:#92400e;background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:10px 14px;margin-bottom:24px">⚠️ <strong>Salve esta senha agora</strong> — ela não será exibida novamente. Você pode alterá-la depois no painel.</p>
    <div style="background:#f8faff;border-radius:12px;padding:20px 24px;margin-bottom:24px;border:1px solid #e2e8f0">
      ...
      <div style="margin-bottom:8px;font-size:14px;color:#475569">1️⃣ Acesse <a href="https://avalie360.vercel.app" style="color:#2563eb;font-weight:600">avalie360.vercel.app</a></div>
      <div style="margin-bottom:8px;font-size:14px;color:#475569">2️⃣ Clique em <strong>"Acessar painel"</strong></div>
      <div style="margin-bottom:8px;font-size:14px;color:#475569">3️⃣ Selecione <strong>${orgName}</strong> na lista</div>
      <div style="font-size:14px;color:#475569">4️⃣ Digite a senha acima e clique em <strong>"Entrar"</strong></div>
    </div>
    <div style="text-align:center;margin-bottom:24px">
      <a href="https://avalie360.vercel.app" style="background:#2563eb;color:white;padding:14px 32px;border-radius:10px;text-decoration:none;font-weight:700;font-size:16px;display:inline-block">Acessar o painel →</a>
    </div>
  ...
  <div style="background:#f1f5f9;padding:16px 36px;text-align:center;font-size:12px;color:#94a3b8">Avalie360 · Conectando Gente · Em conformidade com a LGPD · Lei nº 13.709/2018</div>
</div>
</body></html>`;
